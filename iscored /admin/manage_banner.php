...
if(isset($_REQUEST['btnAdd'])){
	...
	mysql_query("INSERT INTO banners(ban_id, barea_id, ban_name, ban_file, ban_link, ban_startdate, ban_enddate, status_id, btype_id, ban_embed_code) VALUES(".$ban_id.", ".$_REQUEST['bareaid'].", '".$_REQUEST['txtbantitle']."', '".$fileUp."', '".$_REQUEST['txtlink']."', '".$_REQUEST['txtstartdate']."', '".$_REQUEST['txtenddate']."', 1, '".$_REQUEST['btype']."', '".mysql_real_escape_string(str_replace("'", "''", $_REQUEST['ban_embed_code']))."')") or die(mysql_error());
...
elseif(isset($_REQUEST['btnEdit'])){
	...
	mysql_query("UPDATE banners SET ban_name='".$_REQUEST['txtbantitle']."', ban_link='".$_REQUEST['txtlink']."', barea_id=".$_REQUEST['bareaid'].",  ban_startdate = '".$_REQUEST['txtstartdate']."', ban_enddate='".$_REQUEST['txtenddate']."', btype_id='".$_REQUEST['btype']."', ban_embed_code='".mysql_real_escape_string(str_replace("'", "''", $_REQUEST['ban_embed_code']))."' ".$strQuery." WHERE ban_id=".$_REQUEST['ban_id']."") or die(mysql_error());
...
elseif(isset($_REQUEST['action'])){
	if($_REQUEST['action'] == 2){
		...
		if(mysql_num_rows($grs) > 0){
			...
			$btype = $grow->btype_id;
			$ban_embed_code = $grow->ban_embed_code;
		}
	}
	else{
		$FormHead = "Add New ";
			...
			<td align="center"><div align="center" class="msg" style="width:98%"><?php print($strMSG);?></div>
				...
				<table border="0" align="center" cellpadding="1" cellspacing="1" class="FormTables">
					<form name="form1" action="<?php print($_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING']);?>" method="post" enctype="multipart/form-data">
						...
						<tr>
							<td align="right">Embedded Code:</td>
							<td align="left"><textarea name="ban_embed_code" style="width:250px;" class="inputsmallBorder" onFocus="this.className='inputsmallBorder2';" onBlur="this.className='inputsmallBorder';" ><?php print($ban_embed_code);?></textarea></td>
						</tr>
						<tr>
				...
				<table align="center" width="96%" border="0" cellpadding="2" cellspacing="0">
					<form name="frm" method="post" action="<?php print($_SERVER['PHP_SELF']);?>" onSubmit="return chkRequired(this);">
						<?php
							$Query = "SELECT b.*, c.barea_name FROM banners AS b, banner_areas AS c WHERE c.barea_id=b.barea_id ORDER BY b.ban_id DESC";
							$counter=0;
							$limit = 20; 
						...
						<tr>
							<td align="center"><table width="100%" align="center" cellpadding="2" cellspacing="1" class="ListTables">
									<tr>
										<td width="20" align="center" class="TableHeads"><input type="checkbox" name="chkAll" onClick="setAll();"></td>
										<td width="120" align="left" class="TableHeads">Title</td>
										<td width="80" align="center" class="TableHeads">Area</td>
										<td width="118" align="center" class="TableHeads">File</td>
										<td width="75" align="center" class="TableHeads">StartDate</td>
										<td width="75" align="center" class="TableHeads">EndDate</td>
										<td width="30" align="center" class="TableHeads">Edit</td>
									</tr>
									<?php
							$cnt = 0;
							$clsName = "ListRow1";
