...
if(isset($_REQUEST['udt'])){
	if(isset($_REQUEST['btnEdit'])){
		$strQRY="UPDATE categories SET cat_name='".$_REQUEST['txtcat']."' WHERE cat_id = ".$_REQUEST['id'];
...
elseif(isset($_REQUEST['del'])){
	if($_SESSION["UType"]>=3){
		...
		$DelQry="DELETE FROM categories WHERE cat_id = ".$_REQUEST['id'];
		$nResult=mysql_query($DelQry) or die("Unable to Delete Record");
		header("Location: manage_categories.php?op=3");
	}
}
elseif(isset($_REQUEST['btnAdd'])){
	if(!IsExist("cat_name", "categories", "cat_name", $_REQUEST['txtcat'])){
		$MaxID = getMaximum("categories","cat_id");
		$strQRY="INSERT INTO categories(cat_id, cat_name) VALUES($MaxID, '".$_REQUEST['txtcat']."')";
		$nRst=mysql_query($strQRY) or die(mysql_error());
		header("Location: manage_categories.php?op=1");
	}
	else{
		header("Location: manage_categories.php?op=4");
	}
}
else{
	$txtcat = "";
    ...
    <tr>
		...
		<td width="100%" valign="top" class="mainBody">
			<table align="center" width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr><td class="adminMainHead">Categories Management</td></tr>
				...
				<tr>
					<td align="center" valign="top">
						...
						<form name="frm" method="post" action="<?php @print($_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING']);?>" enctype="multipart/form-data">
							...
							<tr>
								<td width="130" align="right">Category:</td>
					...
					<td align="center" valign="top">
						<table width="98%" border="0" cellpadding="2" cellspacing="1" class="ListTables">
							<tr>
								...
								<td class="TableHeads" align="center">Category</td>
								<td class="TableHeads" align="center">Edit</td>
								<td class="TableHeads" align="center">Delete</td>
							</tr>
			<?php
					$counter=0;
					$strQry="SELECT * FROM categories ORDER BY cat_id";
			...
			?>
								...
								<td align="center"><a href="<?php print($_SERVER['PHP_SELF']);?>?id=<?php print($row->cat_id);?>&udt=1" title="Edit">Edit</a></td>
								<td align="center"><a href="<?php print($_SERVER['PHP_SELF']);?>?id=<?php print($row->cat_id);?>&del=1" title="Delete" onClick=" javascript: return confirm('Are you sure you want to delete this record');">Delete</a></td>
							</tr>
			<?php
						}
					}
