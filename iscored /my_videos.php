...
if(isset($_REQUEST['btnUpload'])){
	...
	$source = "temporary/".$_SESSION["uniqueFileName"];
	$destination = "videos/main/".$_SESSION["uniqueFileName"];
	rename($source, $destination);
	// now the file is in Videos/main/...
	// name of the file should be $_SESSION["uniqueFileName"]
	...
	$mfileName = "";
	if (!empty($_SESSION["uniqueFileName"])){
		$getFileName = explode(".", $_SESSION["uniqueFileName"]);
		$tmpFile = $dirName."/".$vidid."_".str_replace(" ", "_", $_SESSION["uniqueFileName"]);
		...
		if(rename("videos/main/".$_SESSION["uniqueFileName"], $tmpFile)){
			// Convert File to FLV
...
elseif(isset($_REQUEST['udt'])){
	if(isset($_REQUEST['btnUpdate'])){
		...
		$new_query = "UPDATE videos SET cat_id = '".$cbocat."', vid_name = '".$vidname."', vid_price = '".$vidprice."', vid_file_path = '".$mfileName."', vid_thumb='".$thumbName."', vid_details = '".$viddetails."', vid_length=".$vidTime." WHERE vid_id = ".$vidid;
...
<!-- Progress Bar -->
    ...
    <script type="text/javascript">
      function uploadFile() {
		var fd = new FormData();
		try{
			fd.append("vFile", document.getElementById('vFile').files[0]);
			fd.append("uniqueFileName", '<?php echo basename((substr(uniqid(),11)).(rand(100, 999)).(substr(uniqid(),11)).(rand(100, 999)));?>'+'_'+document.getElementById('vFile').files[0].name);
		} catch(error){
	...
	<div id="inner">
		...
		?>
					...
					<table width="100%" border="0" cellpadding="2" cellspacing="0">
						<form name="form1" id="form1" method="post" action="<?php print($_SERVER['PHP_SELF']."?".$_SERVER['QUERY_STRING']);?>" >
							<tr>
								<td align="right" width="180">Category: </td>
							...
							<tr>
								...
								<td><input type="text" id="vidname" name="vidname" value="<?php print($vidname);?>" class="input" style="width:280px;"  />
									&nbsp;&nbsp;<img src="images/required.gif" width="9" height="10" border="0" alt="Required Field" title="Required Field" /></td>
							</tr>
							<tr>
								<td align="right">Price: </td>
								<td><input type="text" id="vidprice" name="vidprice" value="<?php print($vidprice);?>" class="input" style="width:100px; text-align: right;" />
									&nbsp;&nbsp;<img src="images/required.gif" width="9" height="10" border="0" alt="Required Field" title="Required Field" /></td>
							</tr>
							<tr>
								<td align="right">Video: </td>
								<td><input type="file" name="vFile" id="vFile" style="width:280px;" />
									&nbsp;&nbsp;<img src="images/required.gif" width="9" height="10" border="0" alt="Required Field" title="Required Field" /></td>
							</tr>
                            <tr>
                            	<td>&nbsp;  </td>
                                <td>
                                    <div id="progressNumber" style="width:40px; float:left; margin-top:15px;"> 0% </div>
                                    <div class="demo" style="flost:right;">
							...
							<tr>
								...
								<td><textarea name="txttags" id="txttags" class="input" style="width:330px; height:60px;"><?php print($txttags);?></textarea></td>
							</tr>
							<tr>
								<td align="right" valign="top" style="padding-top:4px;">Details: </td>
								<td><textarea name="viddetails" id="viddetails" class="input" style="width:330px; height:60px;"><?php print($viddetails);?></textarea>
									&nbsp;&nbsp;<img src="images/required.gif" width="9" height="10" border="0" alt="Required Field" title="Required Field" /></td>
							</tr>
							<tr>
								<td align="right"></td>
								<td height="40"><?php if(isset($_REQUEST['udt'])){?>
									<input type="hidden" id="mfileName" name="mfileName" value="<?php print($mfileName);?>" />
									<input type="hidden" id="thumbName" name="thumbName" value="<?php print($thumbName);?>" />
									<!--<input type="submit" id="btnUpdate" name="btnUpdate" value="Update >> >>" />-->

									<input type="button" id="btnUpdate" name="btnUpdate" value="Update >> >>" onclick="uploadFile()"  />
                                    <input type="hidden" id="btnUpdate" name="btnUpdate" value="btnUpdate" />
									<?php } else{ ?>

									<input type="button" id="btnUpload" name="btnUpload" value="Send >> >>" onclick="uploadFile()"  />
                                    <input type="hidden" id="btnUpload" name="btnUpload" value="btnUpload" />
                                    
									<?php } ?>
									&nbsp;&nbsp;
									<input type="button" name="btnCancel" id="btnCancel" value="Cancel >> >>" onclick="javascript: window.location='my_videos.php';" />
								</td>
							</tr>
						</form>
					</table>
