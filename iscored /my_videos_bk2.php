...
if(isset($_REQUEST['btnUpload'])){
	...
	if (!empty($_FILES["vFile"]["name"])){
		...
		$tmpFile = $dirName."/".$vidid."_".$_FILES["vFile"]["name"];
		$mfileName = $vidid."_".$getFileName[0].".flv";
		$imgFile = $vidid."_".$getFileName[0].".jpg";
		$mediaFile = $dirName."/".$mfileName;
		if(move_uploaded_file($_FILES['vFile']['tmp_name'], $tmpFile)){
			// Convert File to FLV
			if($getFileName[1] != 'flv'){
				$cmdMain = "/usr/local/bin/ffmpeg -i ".$tmpFile." -ab 56 -y -ar 44100 -s 640x480 -f flv ".$mediaFile;
			...
			$cmdThumb = "/usr/local/bin/ffmpeg  -itsoffset -8  -i ".$mediaFile." -vcodec mjpeg -vframes 1 -an -f rawvideo -s 122x69 ".$dirThumb."/".$imgFile;
	...
	mysql_query("INSERT INTO videos (vid_id, mem_id, cat_id, vid_name, vid_price, vid_file_path, vid_thumb, vid_details, vid_dateadded, vstatus_id) VALUES(".$vidid.", ".$_SESSION["UserID"].", '".$cbocat."', '".$vidname."', '".$vidprice."', '".$mfileName."', '".$imgFile."', '".$viddetails."', '".@date("Y-m-d")."', 1)");
...
elseif(isset($_REQUEST['udt'])){
	if(isset($_REQUEST['btnUpdate'])){
		...
		mysql_query("UPDATE videos SET cat_id = '".$cbocat."', vid_name = '".$vidname."', vid_price = '".$vidprice."', vid_file_path = '".$mfileName."', vid_thumb='".$thumbName."', vid_details = '".$viddetails."' WHERE vid_id = ".$vidid);
