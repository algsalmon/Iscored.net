...
if(isset($_REQUEST['btnSearch'])){
	...
	$strHead = "Serarch results for " . $txtSearch;
	$tagSearch = "";
	$tCount = 0;
	$rsTag = mysql_query("SELECT * FROM tags WHERE tag_name like '%".$_REQUEST['txtSearch']."%' ORDER BY tag_total");
	if (mysql_num_rows($rsTag)>=1){
		$tagSearch .= "AND (";
		while($rowTag=mysql_fetch_object($rsTag)){
			if($tCount>0){
				$tagSearch .= " OR ";
			}
			$tagSearch .= "t.tag_id=".$rowTag->tag_id;
			$tCount++;
		}
		$tagSearch .= ")";
	}
	//print($tagSearch);
	if($tCount>0){
		$vidQry = "SELECT DISTINCT v.*, m.mem_name, (SELECT COUNT(vcom_id) FROM vid_comments WHERE vstatus_id=1 AND vid_id=v.vid_id) AS totalComments FROM videos AS v, members AS m, video_tags AS t WHERE (m.mem_id=v.mem_id AND v.vstatus_id=1 ".$tagSearch." AND v.vid_id=t.vid_id) OR (m.mem_id=v.mem_id AND v.vstatus_id=1 AND v.vid_name like '%".$_REQUEST['txtSearch']."%') ORDER BY v.vid_id DESC";
	}
	else{
		$vidQry = "SELECT DISTINCT v.*, m.mem_name, (SELECT COUNT(vcom_id) FROM vid_comments WHERE vstatus_id=1 AND vid_id=v.vid_id) AS totalComments FROM videos AS v LEFT OUTER JOIN members AS m ON m.mem_id=v.mem_id WHERE v.vstatus_id=1 AND v.vid_name like '%".$_REQUEST['txtSearch']."%' ORDER BY v.vid_id DESC";
	}
	//$vidQry = "SELECT DISTINCT v.*, m.mem_name, (SELECT COUNT(vcom_id) FROM vid_comments WHERE vstatus_id=1 AND vid_id=v.vid_id) AS totalComments FROM videos AS v, members AS m, video_tags AS t WHERE m.mem_id=v.mem_id AND v.vstatus_id=1 ".$tagSearch." AND v.vid_id=t.vid_id ORDER BY v.vid_id DESC";
	//print($vidQry);
	//die();
}
else{
...
?>
					<p>Sorry, there are no videos available. Please visit again.</p>
<?php
	}
?>
